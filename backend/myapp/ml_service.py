...
class MLModelService:
    ...
    def load_model(self):
        ...
        try:
            # Use relative path to the model in model folder
            # BASE_DIR is backend/, so we go up one level then to model/
            ...
            if not os.path.exists(model_path):
                # Fallback check
                model_path = os.path.join(os.getcwd(), "model", "best_model.pth")
